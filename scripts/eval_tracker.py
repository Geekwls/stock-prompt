...
台账默认定位：优先仓库根 eval/predictions.jsonl；独立安装运行时固定为
~/.stock-prompt/eval/predictions.jsonl。同一日期重复写入视为更新（后写覆盖）。
"""
...
def default_ledger():
    """定位默认台账，避免随当前工作目录漂移丢失历史。"""
    current = Path(__file__).resolve().parent
    for ancestor in (current, *current.parents):
        if (ancestor / ".agents").is_dir() and (ancestor / "version.json").is_file():
            return ancestor / "eval" / "predictions.jsonl"
    return Path.home() / ".stock-prompt" / "eval" / "predictions.jsonl"
...
def cmd_report(args):
    ...
    print(f"=== market-prediction 滚动评估（最近 {len(pairs)} 个已完成日，窗口 {args.window}，待收盘 {pending} 日，台账 {args.ledger}） ===")
    if not pairs:
...
def main():
    ...
    parser.add_argument(
        "--ledger", default=None,
        help="台账文件路径 (默认: 仓库根 eval/predictions.jsonl；独立安装时 ~/.stock-prompt/eval/predictions.jsonl)",
    )
