...
其中炸板率必须使用该板块内部“触板后未封住家数 ÷ 触板家数”——可执行路径：优先调用 MCP `get_sector_limit_quality`（板块 BK 代码或中文名，支持历史日期），工具内部已完成归属计算。注意涨停池与炸板池的 `hybk` 行业字段是 **≤4 字缩写**（如“农产品加工”→“农产品加”），**禁止按板块全称精确匹配 `hybk`**（会整批漏票、虚高板块炸板率）；MCP 不可用而手工过滤时，必须按“`hybk` 与全称精确相等，或 `hybk` 为全称前缀”规则过滤，且缩写与多个板块前缀冲突时剔除该票并披露。板块触板家数 <3 时 `Q` 记缺失并对可得权重重新归一化（小样本炸板率不稳定），不得借用全市场炸板率。
