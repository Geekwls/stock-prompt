...
def read_project_version():
    search_dirs = [SCRIPTS_DIR] + list(SCRIPTS_DIR.parents)
    for parent in search_dirs:
        for filename in ("version.json", ".stock-prompt-runtime.json", ".stock-prompt-manifest.json", "registry.json"):
            ...
            try:
                ...
                if "latest" in payload:
                    return str(payload["latest"])
                if "project_version" in payload:
                    return str(payload["project_version"])
                if "project" in payload and isinstance(payload["project"], dict):
                    version = payload["project"].get("version")
                    if version:
                        return str(version)
...
def main(argv=None):
    ...
    if not target.is_file():
        # Fallback to repo root scripts/ if running from inside .agents/skills/*/scripts/
        repo_target = SCRIPTS_DIR.parent.parent.parent / "scripts" / SUBCOMMANDS[command]
